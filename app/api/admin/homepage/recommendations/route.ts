...
export async function PUT(request: NextRequest) {
  try {
    ...
    const recommendation = await putByAdminId({
      id,
      importIfEmpty: importRecommendationsFromConfigIfEmpty,
      listOrdered: () => db.linkedInRecommendation.findMany({ orderBy: { orderIndex: 'asc' } }),
      update: (dbId) => db.linkedInRecommendation.update({ where: { id: dbId }, data }),
      create: () => db.linkedInRecommendation.create({ data }),
    });
